...
test.describe('Auth :: Login UX and session behavior', () => {
  test('AC3: authenticated user navigating to /login is redirected home', async ({ page }) => {
  ...
  test('AC4: /login?session_expired=1 shows session-expired banner', async ({ page }) => {
    await page.goto('/login?session_expired=1')

    await expect(page.getByText('Session expired, please log in again.')).toBeVisible()
  })

  test('AC2: invalid credentials show generic error message', async ({ page }) => {
